...
export class FlashcardComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.cardService.getCardsInDeck(currentDeck).subscribe((cards) => {
      ...
      if (cards.length != 0) {
        ...
        this.displayCard(
          this.cardsData[0].cardid,
          this.cardsData[0].question,
          this.cardsData[0].answer,
          0
        );
      } else {
        this.cardsData = [
  ...
  onSubmit(value, currentDeck) {
    ...
    console.log(
      `"cardid":"${cardid}","question":"${question}","answer":"${answer}"`
    );
    if (value.value.cardid == undefined) {
      delete value.value.cardid;
      this.deckService
        .updateDeck(currentDeck, value.value)
        .subscribe((data) => {
      ...
      this.cardService
        .updateCard(this.deckName, { cardid, question, answer } as Card)
        .subscribe((data) => {
          this.cardsData[this.currentCardIndex].question = question;
          this.cardsData[this.currentCardIndex].answer = answer;
        });
    }
  }
  ...
  deleteCard(currCardIndex) {
    ...
    } else if (currCardIndex == 0 && this.deckTotal > 1) {
      if (currCardIndex >= this.deckTotal) {
        this.leftClick(currCardIndex);
      } else {
        this.rightClick(currCardIndex);
  ...
  rightClick(currentIndex) {
    ...
    if (currentIndex != deckTotNumb) {
      ...
      this.displayCard(
        this.cardsData[currentIndex].cardid,
        this.cardsData[currentIndex].question,
        this.cardsData[currentIndex].answer,
        currentIndex
      );
    } else {
      this.displayCard(
  ...
  leftClick(currentIndex) {
    ...
    if (currentIndex != 0) {
      ...
      this.displayCard(
        this.cardsData[deckTotNumb].cardid,
        this.cardsData[deckTotNumb].question,
        this.cardsData[deckTotNumb].answer,
        deckTotNumb
      );
    }
  }
